...
for (let i = 0; i < total; ++i) {
  ...
  for (let j = 0; j < 4; ++j) {
    ...
    if (href) {
      ...
      else if (host == "www.horsetelex.com") {
        let idx = href.indexOf("/", 44) + 1;
        href = " href=" + href.slice(6, idx) + href.slice(idx).split("-").map(v => v.length < 3 ? v : v[0].toUpperCase() + v.slice(1)).join("-");
      }  else if (host == "sporthorse-data.com") 
        href = " href=//sporthorse-data.com/pedigree/" + href.slice(37).split("-").map(v => v.length < 3 ? v : v[0].toUpperCase() + v.slice(1)).join("-");
      else
